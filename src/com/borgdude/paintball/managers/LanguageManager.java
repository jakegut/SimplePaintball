	...
	private File loadLanguage(String loc) {
		...
		if(!langFile.exists()) {
			...
			try {
				...
				plugin.getServer().getConsoleSender().sendMessage(ChatColor.RED + "Language locale " + ChatColor.YELLOW + loc + ChatColor.RED + " not found... Reverting to: " 
						+ ChatColor.YELLOW + DEFAULT_LOCALE);
				if(loc == "enUS") return null;
