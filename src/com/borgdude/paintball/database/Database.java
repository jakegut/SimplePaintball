...
public abstract class Database {
    ...
    public Integer getIntColumn(UUID uuid, String column) {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                if (rs.getString("player_id").equalsIgnoreCase(uuid.toString())) { // Tell database to search for the
                                                                                   // player you sent into the method.
                                                                                   // e.g getTokens(sam) It will look
                                                                                   // for sam.
                    return rs.getInt(column); // Return the players ammount of kills. If you wanted to get total (just a
                                              // random number for an example for you guys) You would change this to
                                              // total!
                }
            }
        } catch (SQLException ex) {
    ...
    public List<PlayerStats> getTop(String column, int limit) {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                stats.add(new PlayerStats(UUID.fromString(rs.getString("player_id")), rs.getInt("kills"),
                        rs.getInt("wins")));
            }
        } catch (SQLException ex) {
            plugin.getLogger().log(Level.SEVERE, Errors.sqlConnectionExecute(), ex);
...
// Now we need methods to save things to the database
    public void setTokens(UUID uuid, Integer kills, Integer wins) {
        ...
        try {
            conn = getSQLConnection();
            ps = conn.prepareStatement("REPLACE INTO " + table + " (player_id,kills,wins) VALUES(?,?,?)"); // IMPORTANT.
                                                                                                           // In SQLite
                                                                                                           // class, We
                                                                                                           // made 3
                                                                                                           // colums.
            ...
            ps.setInt(2, kills); // This sets the value in the database. The colums go in order. Player is ID 1,
                                 // kills is ID 2, Total would be 3 and so on. you can use
                                 // setInt, setString and so on. tokens and total are just variables sent in, You
                                 // can manually send values in as well. p.setInt(2, 10) <-
                                 // This would set the players kills instantly to 10. Sorry about the variable
